(function(window) {
	...
	p.start = function()
	{
		...
		//validate trail value - ensure it's within reasonable bounds
		if (trail < cfg.trailMin || trail > cfg.rollLength) {
			trail = cfg.trail;
		}
	...
	p.setTrail = function(value, dontResetSpeed)
	{
		...
		//ensure trail is within valid bounds
		this.trail = Math.max(cfg.trailMin, Math.min(cfg.rollLength, value));
